...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.x_data_line = self.graphWidget.plot(
            [], [], name="x", pen=pg.mkPen(color=x_data_color)
        )
        self.y_data_line = self.graphWidget.plot(
            [], [], name="y", pen=pg.mkPen(color=y_data_color)
        )
        self.z_data_line = self.graphWidget.plot(
            [], [], name="z", pen=pg.mkPen(color=z_data_color)
        )
        ...
        )  # call update_plot_data function every 50 milisec
        self.timer.start()
...
sensor = Sensor(address="192.168.1.4:8080", sensor_type="android.sensor.accelerometer")
